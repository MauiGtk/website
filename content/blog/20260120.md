...
- Qt is based on C++ along with a special precompiler but we more like the modern C# language MAUI is written in and there is good support for Gtk with GtkSharp and gir.core. GTK has C# bindings such as GtkSharp and Gir.Core, whereas Qt currently lacks an actively maintained C# wrapper.
...
MonoTouch (for iOS) and Mono for Android were licensed by Attachmate as Xamarin.iOS and Xamarin.Android that allowed C# app development on both mobile platforms. Xamarin released Xamarin Studio in 2013 followed by Xamarin.Forms in 2014, which basically made it possible to write C# code that would run on iOS, Android, and the Universal Windows Platform (UWP), supporting also the Windows Phone back then.
